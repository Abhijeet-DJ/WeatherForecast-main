...
router.get("/api/weather", async (req, res) => {
  try {
    ...
    // console.log(aqiEntry["list.0.aqi"].aqi);
    ...
    if (Object.keys(req.query).length > 0) {
      ...
      updateFields["list.0.wind.speed"] =
        ...
        // console.log("We reached error zone");
...
router.get("/weather/3hrs", async (req, res) => {
  ...
  try {
      let dataHourly = await axios.get(urlOpenW);
      dataHourly = dataHourly.data.list.slice(0,3).map(item => item.main.temp);

      const updatedWeather = await WeatherData.findOneAndUpdate(
        { "city.name" : city },
        { $set : {Hourly : dataHourly }},
        { new: true, upsert: true }
      )
      res.status(200).send(updatedWeather)
  } catch (err) {
    console.error("API fetch failed:", err.message);

    // 4️⃣ Fallback: serve existing Hourly data from DB
    try {
      const storedWeather = await WeatherData.findOne({ "city.name": city });

      if (!storedWeather || !storedWeather.Hourly.length) {
        return res.status(500).json({
          error: "No stored hourly data available yet. Try again after first successful fetch."
        });
      }

      return res.status(200).json({
        message: "API fetch failed, returning last stored hourly forecast",
        hourly: storedWeather.Hourly
      });

    } catch (mongoErr) {
      console.error("MongoDB error:", mongoErr.message);
      return res.status(500).json({ error: "Failed to fetch hourly data from DB" });
    }
  }
});
